...
long ct_report(Test * pTest)
{
    ...
    if (pTest->pStream) {
        fprintf(pTest->pStream,
            "<test-case label=\"\" name=\"%s\" skip=\"0\" "
            "failures=\"%ld\" tests=\"%ld\">\n",
            pTest->name, pTest->nFail, pTest->nPass + pTest->nFail);
        fprintf(pTest->pStream, "</test-case>\n");
    }
    return pTest->nFail;
}
    ...
    long line)
{
    ...
    if (pTest->pStream) {
        fprintf(pTest->pStream,
            "<syserr>%s failure: (%s), %s (line %ld)<syserr>\n",
